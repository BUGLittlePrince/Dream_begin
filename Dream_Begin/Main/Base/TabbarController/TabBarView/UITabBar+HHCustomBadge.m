...
static NSString * const kBadgeViewInitedKey = @"kBadgeViewInitedKey";
...
@implementation UITabBar (HHCustomBadge)
...
- (void)addBadgeViews
{
    ...
    //number views
    NSMutableArray *badgeNumViews = [NSMutableArray new];
    for (int i = 0; i < itemsCount; i ++) {
        ...
        redNum.font = [UIFont systemFontOfSize:12.0];
        redNum.textColor = [UIColor whiteColor];
        
        [self addSubview:redNum];
        [badgeNumViews addObject:redNum];
    }
    [self setValue:badgeNumViews forKey:kBadgeNumberViewsKey];
}
