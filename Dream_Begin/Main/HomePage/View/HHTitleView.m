...
//标题标签数组
@property (nonatomic, strong) NSMutableArray <UILabel *> *titleLabelsArray;
//滚动视图
...
//选中颜色
@property (nonatomic) const CGFloat *seletedColorRGB;
...
#pragma mark --懒加载--
- (NSMutableArray<UILabel *> *)titleLabelsArray{
    if (_titleLabelsArray == nil) {
        _titleLabelsArray = [NSMutableArray array];
    }
    return _titleLabelsArray;
}
...
- (UIView *)bottomLine{
    if (_bottomLine == nil) {
        ...
        _bottomLine.backgroundColor = CNavBgColor;
    }
    return _bottomLine;
}
...
- (void)setupTitleLabelFrame{
    ...
    if (self.titleLabelsArray.count <= 5)
    {
        titleW = self.frame.size.width / self.titleLabelsArray.count;
        self.style.isTitleViewScrollEnable = NO;
    }else
    {
        titleW = self.frame.size.width / 5;
    ...
    for (int i = 0; i < self.titleLabelsArray.count; i++) {
        ...
        if (self.style.isTitleViewScrollEnable) {//不需要滑动
            ...
            if (i == 0) {
                ...
                UILabel *preLabel = self.titleLabelsArray[i - 1];
        ...
        if (self.style.isTitleViewScrollEnable) {
            self.scrollView.contentSize = CGSizeMake(CGRectGetMaxX(self.titleLabelsArray.lastObject.frame) + self.style.titleMargin * 0.5, 0);
        }
    }
}
...
- (void)setupBottomLine{
    ...
    self.bottomLine.frame = CGRectMake(self.titleLabelsArray.firstObject.frame.origin.x, self.bounds.size.height - self.style.bottomLineH, self.titleLabelsArray.firstObject.frame.size.width, self.style.bottomLineH);
}
...
- (void)setupCoverView{
    ...
    UILabel *firstLabel = self.titleLabelsArray.firstObject;
...
- (void)titleLabelClick:(UITapGestureRecognizer *)tap{
    ...
    if (currentLabel.tag - 100 == self.currentIndex) {
        //        NSLog(@"相同的indnex");
        return;
    }
    //获取之前的Label
    UILabel *oldLabel = self.titleLabelsArray[self.currentIndex];
    ...
    //通知代理
    if ([self.myDelegate respondsToSelector:@selector(titleViewWithTitleView:selectedIndex:)]) {
        [self.myDelegate titleViewWithTitleView:self selectedIndex:self.currentIndex];
    }
...
- (void)setTitleWithProgress:(CGFloat)progress sourceIndex:(NSInteger)sourceIndex targetIndex:(NSInteger)targetIndex{
    ...
    UILabel *sourceLabel = self.titleLabelsArray[sourceIndex];
    UILabel *targetLabel = self.titleLabelsArray[targetIndex];
    //颜色渐变
...
- (void)contentViewDidEndScroll{
    ...
    //获取目标Label
    UILabel *targetLabel = self.titleLabelsArray[self.currentIndex];
