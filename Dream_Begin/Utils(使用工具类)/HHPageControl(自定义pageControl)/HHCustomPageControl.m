...
@implementation HHCustomPageControl
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _pageControlType = HHPageControlTypeSquare;
        _pageControlAlignment = HHPageControlAlignmentDefault;
...
- (void)resetUIView
{
    ...
    if (_pageControlType == HHPageControlTypeLine)
    {
        // 线条样式时，高度最大为2.0
//        self.pageSizeHeight = (2.0 <= self.pageSizeHeight ? 2.0 : self.pageSizeHeight);
    ...
    if (_pageControlAlignment == HHPageControlAlignmentLeft)
    {
        originX = _pageMargin;
    }
    else if (_pageControlAlignment == HHPageControlAlignmentRight)
    {
        originX = self.frame.size.width - (self.pageSizeWidth * _numberOfPages + _pageMargin * _numberOfPages);
    }
    else if (_pageControlAlignment == HHPageControlAlignmentEqual)
    {
        _pageMargin = (self.frame.size.width - self.pageSizeWidth * _numberOfPages) / (_numberOfPages + 1);
        originX = _pageMargin;
    }
    else if (_pageControlAlignment == HHPageControlAlignmentBottom)
    {
        if (_pageControlType == HHPageControlTypeLine)
        {
    ...
    [self.pageNormals enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        // 页码显示样式
        if (_pageControlType == HHPageControlTypeCircle)
        {
            imageViewNormal.layer.cornerRadius = self.pageSizeHeight / 2;
            imageViewSelected.layer.cornerRadius = self.pageSizeHeight / 2;
        }
        else if (_pageControlType == HHPageControlTypeLine)
        {
            // 线条样式时，不显示序号
            _showPageNumber = NO;
            
        }
        else if (_pageControlType == HHPageControlTypeImage)
        {
...
- (CGFloat)pageSizeWidth
{
    ...
    else if (_pageControlType == HHPageControlTypeImage && _shouldAutoresizingImage)
    {
        // 图片形
...
{
    if (_pageControlType == HHPageControlTypeImage && _shouldAutoresizingImage)
    {
        // 图片形
...
- (void)setPageControlType:(HHPageControlType)pageControlType
{
    _pageControlType = pageControlType;
    [self setNeedsLayout];
}

- (void)setPageControlAlignment:(HHPageControlAlignment)pageControlAlignment
{
...
{
    return ^(HHPageControlAlignment alignment) {
