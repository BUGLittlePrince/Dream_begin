...
- (void)pushAnimateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    UIViewController<HHXYTransitionProtocol> *fromVC = (UIViewController<HHXYTransitionProtocol> *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController<HHXYTransitionProtocol> *toVC = (UIViewController<HHXYTransitionProtocol> *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    ...
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        fromView.frame = CGRectMake(-(nowViewPoint.x) * _animationScale, -(nowViewPoint.y) * _animationScale + toViewPoint.y, fromVC.view.frame.size.width, fromVC.view.frame.size.height);
...
//    CGFloat offsetY = fromVC.navigationController.navigationBarHidden ? 0.0 : 64;
    ...
    toView.frame = CGRectMake(-(leftUperPoint.x * _animationScale), -((leftUperPoint.y - nowViewPoint.y) * _animationScale + nowViewPoint.y), toView.frame.size.width, toView.frame.size.height);
